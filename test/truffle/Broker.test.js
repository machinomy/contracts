...
const MerkleTree_1 = require("../../src/MerkleTree");
chai.use(asPromised);
const assert = chai.assert;
const web3 = global.web3;
async function signatureDigest(address, digest) {
...
contract('Broker', accounts => {
    ...
    let channelValue = new bignumber_js_1.default(web3.toWei(1, 'ether'));
    let preimage = support_1.randomUnlock();
    let instance;
    before(async () => {
        if (!instance) {
    ...
    async function openChannel(instance, _settlementPeriod) {
    ...
    async function paymentDigest(address, channelId, merkleRoot) {
        let hash = abi.soliditySHA3(['address', 'uint32', 'bytes32', 'bytes32'], [address, await chainId(), channelId, merkleRoot]);
        return util.bufferToHex(hash);
    }
    async function startSettling(instance, channelId, origin) {
        return instance.startSettling(channelId, { from: origin });
    }
    async function readChannel(instance, channelId) {
    ...
    async function packHashlock(channelId, hashlock) {
        let hashlockBuffer = abi.soliditySHA3(['uint32', 'bytes32', 'bytes32', 'int256'], [(await chainId()), channelId, hashlock.preimage, hashlock.adjustment.toString()]);
        return util.bufferToHex(hashlockBuffer);
    }
    async function combineHashlocks(channelId, ...elements) {
    ...
    async function sign(origin, digest) {
    ...
    async function signPayment(origin, channelId, merkleRoot) {
    ...
    async function merkle(channelId, _amount) {
        let payment = _amount || channelValue;
        let hashlocks = await combineHashlocks(channelId, [preimage, payment]);
        let merkleTree = new MerkleTree_1.default(hashlocks);
    ...
    describe('withdraw', () => {
        let amount = new bignumber_js_1.default(web3.toWei(0.01, 'ether'));
    ...
    describe('settle', () => {
        ...
        async function updateChannel(channelId, value) {
            let channel = await readChannel(instance, channelId);
